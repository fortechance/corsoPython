...
layout_main = [
    ...
    [sg.Text('Tipo movimento  :'), sg.Input(size=(1, 1), key='-TIPO-', tooltip='E=Entrata U=Uscita A=Altro'), sg.Text("    E=Entrata U=Uscita A=Altro")],
    [sg.Text(" ")],
    [sg.HorizontalSeparator()],
...
layout_listbox = [
    [sg.Button(image_filename="F:\img2023\lg2023.png", image_size=(70, 30), border_width=0, button_color=('white')), sg.Text('                ELENCO CAUSALI', font=('Arial Bold', 16))],
    [sg.Listbox(values=[f"{k} - {v}" for k, v in causali_data.items()],  text_color="#4da6ff", size=(40, 6), key='-LISTBOX-', enable_events=True) ]
]
...
while True:
    ...
    if event_main == sg.WINDOW_CLOSED or event_main == 'Uscita':
        ...
        if input3_value.upper() in ['E', 'U', 'A']:
            ...
            sg.popup(f'Inseriti : Cod. causale: {input1_value}, Descrizione: {input2_value}, Tipo movimento: {tipo_movimento}')
        else:
